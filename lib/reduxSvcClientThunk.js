...
function getSvcDefinitionActions(svcDefinition) {
  return _.mapValues(svcDefinition, obj => {
    ...
    return _.mapValues(obj, subObj => {
      if (!_.isPlainObject(subObj)) {
        throw new TypeError('Traversing non-object for actions, check svcDefinition is ');
      }

      if (!_.has(subObj, 'endpoint')) {
        throw new TypeError('this is supposed to be an endpoint configuration');
      }

      return subObj.actions;
    });
  });
}
